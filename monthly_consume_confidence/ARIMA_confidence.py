...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug  2 16:25:15 2022
...
model = sm.tsa.arima.ARIMA(train['value'], order=(2,1,1))
model = model.fit()
#print(model.summary())
...
model2 = sm.tsa.arima.ARIMA(df2['value'], order = (2,1,1))
model2 = model2.fit()
print(df2.tail())
...
pred = model2.predict(start = len(df2), end = len(df2) + 13, typ = 'levels').rename('ARIMA Predictions')
